...
const ClassDetailPage = () => {
  ...
  const user = JSON.parse(localStorage.getItem("user")); // giả định bạn có lưu
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            ...
            <p className="text-gray-600">
              Đây là lớp học thuộc course <b>{classData.course?.title}</b>.
            ...
            <button
              ...
              className={`w-full py-3 rounded-md text-white font-semibold transition ${
                isEnrolled
                  ...
                  : "bg-[#A435F0] hover:bg-purple-700"
              }`}
            >
              {isEnrolled ? "Đã tham gia" : "Tham gia lớp"}
            </button>

            {/* INFO */}
            <div className="mt-4 text-sm text-gray-600 space-y-2">
              <p>📚 Course: {classData.course?.title}</p>
              <p>👨‍🏫 Instructor: {classData.instructor?.name}</p>
              <p>👥 {classData.students.length} học viên</p>
            </div>
          </div>
